...
@st.cache_data
def load_data():
    """Load data from trip_analytics view."""
    conn = get_connection()
    try:
...
if df.empty:
    ...
    weekday_options = df['pick_weekday'].unique() if 'pick_weekday' in df.columns else []

    year = st.sidebar.selectbox("Year", year_options)
    month = st.sidebar.selectbox("Month", month_options)
    weekday = st.sidebar.multiselect("Weekday", weekday_options)
...
st.markdown("---")
st.markdown("Built with Apache Airflow, DuckDB, Streamlit, and Plotly")
'''
