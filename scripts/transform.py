...
"""
Transform raw NYC Uber/Taxi data into Star Schema.

Creates four tables:
1. datetime_dim - Temporal dimension with pickup timestamps and components
2. rate_code_dim - Rate code dimension with descriptions
3. location_dim - Location dimension for pickup/dropoff zones
4. fact_table - Fact table containing trip metrics and foreign keys
"""
...
def transform_data(**context) -> dict:
    ...
    print(f"Transforming {len(df):,} rows after cleaning...")
    ...
    # Save transformed data to temporary files
    ...
    print(f"datetime_dim: {len(datetime_dim):,}")
    print(f"rate_code_dim: {len(rate_code_dim):,}")
    print(f"location_dim: {len(location_dim):,}")
    print(f"fact_table: {len(fact_table):,}")
