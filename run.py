...
def cmd_setup() -> bool:
    ...
    run_command(f'"{PYTHON}" setup_project.py')

    print_info("Setting up pipeline files...")
    run_command(f'"{PYTHON}" setup_pipeline.py')
...
def cmd_start() -> bool:
    ...
    success, _, stderr = run_command("docker-compose up -d")
...
def cmd_verify() -> bool:
    ...
    for phase in phases:
        ...
        success, _, stderr = run_command(f'"{PYTHON}" verify-phase-{phase}.py')
    ...
    print_warning("Some verifications failed. Fix them before proceeding.")
    return False


def cmd_verify_phase(phase: int) -> bool:
...
def cmd_trigger() -> bool:
    ...
    success, _, stderr = run_command("airflow dags trigger uber_etl_pipeline")
